...
# Explicitly load from key.env (your custom file)
load_dotenv(dotenv_path="key.env", override=True)

# 1️⃣ Try Streamlit secrets first (when deployed)
GOOGLE_API_KEY = None
try:
...
# 2️⃣ Fallback: environment variable from key.env
if not GOOGLE_API_KEY:
    GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY")

# 3️⃣ Fail safely if not found
if not GOOGLE_API_KEY:
    st.error("❌ GOOGLE_API_KEY not found.\n\n➡️ Please add it in **key.env** (local Jupyter/Streamlit) or `.streamlit/secrets.toml` (Streamlit Cloud).")
    st.stop()
